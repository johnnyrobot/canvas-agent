...
| `renderer/ui-theme.ts`, `renderer/catalog-view.ts`, `renderer/model-health.ts` | ✅ | DOM-free view helpers (theme class resolution, catalog labels/prompt lines, which REQUIRED models are missing and what the download affordance says — ADR-0009). Pure functions over a snapshot — unlike `remediate-review.ts`, which owns state; see ADR-0002 for why the two shapes differ. |
