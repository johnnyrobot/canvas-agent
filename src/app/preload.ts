 ...
 * three channels in `channels.ts`.
 *
 * The actual bridge object is built by the pure, unit-tested `createBridge`
 * (`bridge.ts`); this file only binds it to `ipcRenderer.invoke` and publishes
 * it. That keeps the Electron import confined here and out of the test path.
 */
