...
|---|---|
| **IBM Granite** LLM weights | Pulled at runtime by Ollama; the user obtains them directly. Apache-2.0, so no acceptable-use pass-through (ADR-0007). Not bundled. |
...
1. **Gemma user-facing terms — SUPERSEDED (2026-08-08, ADR-0007).** The app no
   longer defaults to Gemma, so no third-party acceptable-use policy attaches to
   the default path and the obligation this item recorded no longer exists. Every
   shipped default is now required to be permissively licensed, enforced by a
   guard test. The renderer's footer notice remains, restated for the Granite
   models it actually runs (`src/app/renderer/index.html`).
...
(legal/business) decision** — in particular confirming comfort with the
weak-copyleft components (MPL-2.0 `certifi`/`tqdm`, LGPL-2.1
